...
1. The script `cross_surprisal_acl_update.py` can be run locally now with the `create_bible_dict` function, avoiding the `extract_features` class. Download the folder `ENGKJV` and change the `self.root` path. Use `test` set to `True` for running single paragraphs. Choose a number between 1 and 260 at lines 232 and 233.
